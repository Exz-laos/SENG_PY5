...
# ---------------------------------------------------------
st.set_page_config(page_title="digitalSignal final1", page_icon="📝", layout="centered")
...
# Filter questions based on range and OK status
filtered_questions = []
...
if st.sidebar.button("🎲 Generate New Quiz", type="primary", use_container_width=True):
    if filtered_questions:
        ...
        st.sidebar.error("No questions available! Adjust range or uncheck 'Exclude OK'.")

# ---------------------------------------------------------
# OK Manager (Sidebar)
# ---------------------------------------------------------
st.sidebar.divider()
st.sidebar.header("✅ OK'd Questions Manager")

if st.session_state.ok_ids:
    st.sidebar.success(f"You have mastered **{len(st.session_state.ok_ids)}** questions.")
